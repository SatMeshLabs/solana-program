...
#[derive(Accounts)]
#[instruction(_event_id: u64, _event_bump: u8, _mint_position: u64, _spot_nft_bump: u8)]
pub struct BurnSpotContext<'info> {
    #[account(mut)]
...
pub fn handler(ctx: Context<BurnSpotContext>, _event_id: u64, _event_bump: u8, _mint_position: u64,_spot_nft_bump: u8) -> Result<()> {
    ...
    let transfer_instruction = anchor_spl::token::Transfer {
        ...
        to: ctx.accounts.token_ata_sender.to_account_info(),
        authority: ctx.accounts.event_token_account.to_account_info(),
    };

    // seeds = [b"event-asset".as_ref(), _event_id.to_le_bytes().as_ref()],
    let bump_vector = _event_bump.to_le_bytes();
    let binding = _event_id.to_le_bytes();
    let inner = vec![b"event-asset".as_ref(),binding.as_ref(), bump_vector.as_ref()];
